...
def papelera():
    estudiantes = Estudiante.query.filter_by(eliminado=True).all()
    return jsonify([e.to_dict() for e in estudiantes]), 200
...
@app.route("/papelera/<int:id>", methods=["DELETE"])
...
def vaciar_papelera():
    Estudiante.query.filter_by(eliminado=True).delete()
    db.session.commit()
